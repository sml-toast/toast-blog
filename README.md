...
加密访问的后台数据管理面板，支持多环境隔离、多语言切换、备份还原。

### 功能总览

| 功能 | 说明 | 状态 |
|------|------|------|
| 🔑 密码保护 | 访问 `/admin.html` → 输入密码 `admin` | ✅ |
| 🟢DEV 🟠TEST 🔵PROD | 三环境数据隔离，切换带色块标签 + 加载动画 | ✅ |
...
| 🌐 多语言 | 启用/禁用/默认语言/支持语言列表 | ✅ |
| 📥 导出 JSON | 一键导出所有数据为 JSON 备份 | ✅ |
| 📤 导入 JSON | 从 JSON 文件恢复数据 | ✅ |
| 💾 备份 | 导出完整数据包 + 附件清单，存到 `data/{env}/` | ✅ |
| 📂 还原 | 选择备份文件恢复到当前环境 | ✅ |
| 🔄 重置默认 | 恢复为静态文件默认数据 | ✅ |
| 🔐 登录缓存 | localStorage + 30min 过期，退出清除 | ✅ |
| 🚪 退出 | 清除登录缓存，返回登录页 | ✅ |
| 📝 开发日志 | 每完成一个功能自动记录到 `DEVELOPMENT_LOG.md` | ✅ |

### 数据流

```
后台操作 → admin.js → data/loader.js → localStorage({env})
                                              ↓
...
上传的图片和附件以 base64 格式存储在 localStorage。
备份时导出为结构化 JSON，包含文件清单。
